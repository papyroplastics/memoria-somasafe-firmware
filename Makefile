...
.PHONY: shared monitor debug gfb qemu format test-model serial-write
shared:
...
test-model:
	uv run -m scripts.test_model shared/gen/models/feature-mlp/quantized.tflite \
		shared/gen/exports/S1.ssds --results=100

serial-write:
	uv run -m scripts.serial_write ${port_uart} shared/gen/exports/S1.ssds --rate=900
