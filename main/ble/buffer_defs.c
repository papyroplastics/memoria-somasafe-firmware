...
bool ble_buffer_defs_register(
  ...
  for (size_t i = 0; i < gatt_buffer_services_len; i++) {
    ...
    if (ble_uuid_cmp(svc_uuid, &service->svc_uuid.u) == 0 &&
        ble_uuid_cmp(chr_uuid, &ble_buffer_chr_uuid.u) == 0) {

      if (ble_uuid_cmp(dsc_uuid, &ble_buffer_size_dsc_uuid.u) == 0) {
        service->size_dsc_handle = dsc_handle;
        return true;
      }

      if (ble_uuid_cmp(dsc_uuid, &ble_buffer_pos_dsc_uuid.u) == 0) {
