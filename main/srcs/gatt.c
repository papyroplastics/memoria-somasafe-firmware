...
static uint16_t hr_chr_attr_handle;

static int hr_chr_access(uint16_t conn_handle, uint16_t attr_handle,
    struct ble_gatt_access_ctxt *ctxt, void *arg) {

  if (likely(ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR)) {
    if (attr_handle != hr_chr_attr_handle) {
    ...
    if (conn_handle == BLE_HS_CONN_HANDLE_NONE) {
      ESP_LOGI(tag, "nimble stack read value %d from HR chr (handle %d)",
          hr_chr_value[1], attr_handle);
    } else {
      ESP_LOGI(tag, "client %d read value %d from HR chr (handle %d)",
          conn_handle, hr_chr_value[1], attr_handle);
    }
...
static const struct ble_gatt_svc_def gatt_svcs[] = {
  {
    ...
    .characteristics = (struct ble_gatt_chr_def[]) {
      {
        .uuid = &hr_chr_uuid.u,
        .access_cb = hr_chr_access,
        .flags = BLE_GATT_CHR_F_READ | BLE_GATT_CHR_F_INDICATE,
        .val_handle = &hr_chr_attr_handle
      },
      {0}
    }
  },
...
int gatt_hr_attr_signal() {
  if (hr_subs_conn_handle == 0) {
    return 1;
  }

  int err = ble_gap_conn_find(hr_subs_conn_handle, NULL);
  if (err != 0) {
    ESP_LOGE(tag, "unable to find client %d for HR signal", hr_subs_conn_handle);
    hr_subs_conn_handle = 0;
    return 1;
  }

  err = ble_gatts_indicate(hr_subs_conn_handle, hr_chr_attr_handle);
  if (err != 0) {
    ESP_LOGE(tag, "error seding HR signal to client %d", hr_subs_conn_handle);
    return 1;
  }
